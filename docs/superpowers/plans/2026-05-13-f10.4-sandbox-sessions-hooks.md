...
func decodePayload(r *http.Request) (map[string]any, error) {
	...
	dec.DisallowUnknownFields() // we accept unknown fields actually — REMOVE if too strict
...
**NOTE for implementer:** the `DisallowUnknownFields` line will reject claude payloads that have extra fields like `cwd`, `session_id`, etc. **Remove that line** before shipping. The implementer should leave `dec.Decode(&p)` plain.
...
func fmtSession(s Session) string { return fmt.Sprintf("Session{ID:%s,Status:%s}", s.ID, s.Status) }
```

The `fmtSession` helper at bottom is only for log debug — remove if `staticcheck` flags unused.
...
func setupSessionsFixture(t *testing.T) (
	*sql.DB, string, *fakeRuntime, *hooks.TokenRegistry, *events.FakeEmitter, *SessionsService, string,
	...
	svc := NewSessionsService(
		sessRepo, wtRepo, projectsRepo, wtSvc,
...
func NewSessionsService(
	...
	return &SessionsService{
		sessions: sessions, worktrees: worktrees, projects: projects, wtSvc: wtSvc,
...
// taskByID is a thin helper since the service doesn't otherwise hold a
// TasksRepo reference; query through projects for the embedded task row.
// Implementer: inject store.TasksRepo into NewSessionsService and use that
// directly. (Adjust constructor signature accordingly.)
func (s *SessionsService) taskByID(ctx context.Context, taskID string) (*store.Task, error) {
	return nil, errors.New("TODO: wire store.TasksRepo into SessionsService and replace this")
}
...
**Note for implementer:** the `taskByID` placeholder above means SessionsService also needs `store.TasksRepo` injected. Add it to the struct and constructor before this compiles. The plan acknowledges this as a small adjustment the implementer must make to align with the test fixture (which already constructs `tasksRepo`).
...
sessionsSvc := core.NewSessionsService(
	sessionsRepo, worktreesRepo, projectsRepo, worktreesSvc,
...
- [ ] **Step 3: Update `cmd/jarvis-e2e-http/main.go`** to call `srv.SetHookBase(hookServer.BaseURL())` after both `hookServer.Start()` and `srv.Start()`.
...
```bash
git add internal/api/http_e2e.go cmd/jarvis-e2e-http/main.go
git commit -m "feat(F10.4.20): /e2e/sessions/* endpoints + simulate_hook proxy to hook server"
```
...
export async function getSessionToken(sessionId: string): Promise<string> {
  // Internal endpoint that exposes the in-memory token for the test only.
  // (Implementer: add a debug route /e2e/sessions/__token in http_e2e.go that
  //  reads from the registry — guarded behind build tag e2e_http; never in prod.)
  const port = e2ePort();
...
**Implementer addendum:** Add `__token` debug endpoint to `internal/api/http_e2e.go` (only behind `e2e_http` tag) that resolves a session_id → token by iterating the token registry. This is the only way Playwright tests can simulate hooks for a specific session.
