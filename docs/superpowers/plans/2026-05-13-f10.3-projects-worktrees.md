...
// shell runs a /bin/sh command inside dir. Fatals on non-zero exit.
// Used to seed git repos for ops tests.
func shell(t *testing.T, dir, cmd string) {
	t.Helper()
	out, err := exec.Command("sh", "-c", cmd).CombinedOutput()
	if err != nil {
		t.Fatalf("shell in %s: %v\nout: %s", dir, err, string(out))
	}
}
...
func initRepo(t *testing.T) string {
	t.Helper()
	d := t.TempDir()
	shell(t, d, "cd "+d+" && git init -q && git -c user.email=t@t -c user.name=t commit -q --allow-empty -m init")
	return d
}
...
func newTestDB(t *testing.T) *store.DB { /* shared helper — see existing pattern */ }

// We test against the REAL `git` binary in t.TempDir() since detect_repos
// is pure-FS and uses no subprocess. (No fake needed for projects.go.)
...
func TestProjectsService_Create_Monorepo(t *testing.T) {
	db := newTestStoreDB(t) // alias to internal helper that opens + migrates
	bus := &events.FakeEmitter{}
	svc := NewProjectsService(
		store.NewProjectsRepo(db), store.NewRepositoriesRepo(db), bus,
	)
...
func TestProjectsService_Create_MultiRepo(t *testing.T) {
	...
	// Sorted by sub_path ASC; alpha then beta.
...
`newTestStoreDB(t)` is a tiny helper that wraps the existing `store.newTestDB(t)` exported as `store.NewTestDB(t)` — add this helper-export in store first if it doesn't exist. (Inspect `internal/store/tasks_test.go` for the current `newTestDB` signature.)
...
func (s *WorktreesService) CleanupForTask(ctx context.Context, taskID string) error {
	...
	var emits []events.EmitCall
	for _, w := range wts {
		repo, err := s.repos.ListByProject(ctx, "") // we need repo path; load directly
		_ = repo
		_ = err
		// We need project path to feed git. Look up the repo row, then the project.
		// Inline these queries; not worth abstracting yet.
		var subPath, projectPath, projectID string
		row := s.worktrees.GetByID
		_ = row
		// Simpler: directly query the joined info.
		var err2 error
		subPath, projectPath, projectID, err2 = s.repoAndProjectPath(ctx, w.RepositoryID)
		if err2 != nil {
			// Bookkeeping failure — orphan to be safe, keep going.
			_ = s.worktrees.OrphanRow(ctx, w.ID)
			emits = append(emits, orphanedEmit(projectID, w.ID, w.Path, err2.Error()))
			continue
		}
...
- [ ] **Step 1: Update tests to use fake ProjectsService surface**

Replace the existing `fakeProjectsRepo` with a fake service. The test file currently fakes `ProjectsRepo` directly — we now want it to fake `*core.ProjectsService` indirectly via an interface the API depends on.
...
- [ ] **Step 1: Add worktree + fixture endpoints to `internal/api/http_e2e.go`**
...
test.describe('projects + worktrees flow', () => {
  ...
  test('discard task with worktree triggers cleanup (Go side)', async ({ page }) => {
    await openApp(page);
    ...
    // Manually add worktree linked to the task — for now we'd need a /e2e/worktrees/__seed
    // endpoint; if not implemented, this test asserts only that discard succeeds end-to-end.
    await page.request.post(`http://127.0.0.1:${port}/e2e/tasks/discard`, {
      data: { id: task.id },
    });
