...
async def test_list_tasks_filter_project_ids(db_session, tmp_path) -> None:
    ...
    await create_task(db_session, project_id=pid_b, title="B1")
...
async def test_create_task_project_not_found_raises(db_session) -> None:
    with pytest.raises(ProjectNotFoundForTaskError):
        await create_task(db_session, project_id="nonexistent", title="T")


async def test_update_task_description(db_session, tmp_path) -> None:
    pid = await _seed_project(db_session, tmp_path)
    t = await create_task(db_session, project_id=pid, title="X")
...
async def test_update_task_invalid_transition_raises(db_session, tmp_path) -> None:
    pid = await _seed_project(db_session, tmp_path)
    t = await create_task(db_session, project_id=pid, title="X")
